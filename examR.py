...
'''
'''
ElasticNet — линейная регрессия с комбинированной L1 и L2 регуляризацией (LASSO + Ridge).

Краткое описание:
ElasticNet сочетает в себе признаки L1-регуляризации (как в LASSO, которая может обнулять коэффициенты) и L2-регуляризации (как в Ridge, которая уменьшает коэффициенты, но не обнуляет их).

Параметры в вашем коде:

alpha (общая сила регуляризации): чем больше, тем сильнее штраф за большие коэффициенты.

l1_ratio (соотношение L1/L2):

0 = чистая Ridge-регуляризация,

1 = чистая LASSO-регуляризация,

0.2–0.8 = смесь двух типов.
'''